/*
PCA9536_PString.h - Lightweight printable string class
...
class PCA9536_PString : public Print {
    private:
        char *_buf, *_cur;
        size_t _size;
    public:
        ...
        PCA9536_PString(char *buf, size_t size) : _buf(buf), _size(size) {
            begin();
        }
    
        // templated constructors allow inline renderings of this type: PCA9536_PString(buf, size, myfloat[, modifier]);
        template<class T> PCA9536_PString(char *buf, size_t size, T arg) : _buf(buf), _size(size) {
            begin();
            print(arg);
        }

        template<class T> PCA9536_PString(char *buf, size_t size, T arg, int modifier) : _buf(buf), _size(size) {
            begin();
            print(arg, modifier);
        }
        ...
        template<class T> inline PCA9536_PString &operator =(T arg) {
            begin();
            print(arg);
            return *this;
        }
